...
//    TimerTool timerTool;
    ...
    public String getToken() {
        ...
        weather2 = weatherTool.getWeather("101090201");
        return token;
    }
...
//        System.out.println("进入重复执行程序");
        ...
        if (hour < 15) {
            //早上模板
            template_id = "lzLs2dgKOxmvVAPb3dne-p4MxsEfszj9jxqqeNiWVJ4";
        } else {
            //晚上模板
            template_id = "tY58D0vd2N7ZbVB2xohjuy158ZEYS5vpXz3Sbo8izCc";
        }
        //整体参数map
        ...
        //下面是天气数据
        if (userID.equals("okVyw6vkgaPm7OQ5GQOpdRernMHs")) {
            weather = weather1;
        } else {
            weather = weather2;
        }
...
//                System.out.println("进入定时器,当前时间：" + simpleDateFormat.format(System.currentTimeMillis()));
                ...
                try {
                    if (hour == 22 && minutes == 50 && second == 0) {
                        SendWeChatMsg(getToken(), "okVyw6vkgaPm7OQ5GQOpdRernMHs");
                        SendWeChatMsg(getToken(), "okVyw6pf2Bl8OTzeegC-FNqV8Oxk");
                    } else if (hour == 13 && minutes == 30 && second == 0) {
                        SendWeChatMsg(getToken(), "okVyw6vkgaPm7OQ5GQOpdRernMHs");
                        SendWeChatMsg(getToken(), "okVyw6pf2Bl8OTzeegC-FNqV8Oxk");
                    }
